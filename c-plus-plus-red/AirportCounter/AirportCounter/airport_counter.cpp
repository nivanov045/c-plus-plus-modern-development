...
public:
  ...
  {
    return numberOfDepartures[static_cast<size_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) { ++numberOfDepartures[static_cast<size_t>(airport)]; }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) { --numberOfDepartures[static_cast<size_t>(airport)]; }

  // удалить все вхождения данного элемента
  void EraseAll(TAirport airport) { numberOfDepartures[static_cast<size_t>(airport)] = 0; }
  ...
  Items GetItems() const
  {
    ...
    for (size_t i = 0; i < static_cast<size_t>(TAirport::Last_); ++i) {
      ...
      result[i].second = numberOfDepartures[i];
    }
    return result;
  }

private:
  array<size_t, static_cast<size_t>(TAirport::Last_)> numberOfDepartures;
};
