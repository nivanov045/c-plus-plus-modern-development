...
void Stats::operator+=(const Stats& other)
{
  for (auto& w : word_frequences) {
    if (other.word_frequences.count(w.first))
      w.second += other.word_frequences.at(w.first);
  }
  for (auto& w : other.word_frequences) {
    if (!word_frequences.count(w.first))
      word_frequences.insert({ w.first, w.second });
  }
}
...
Stats ExploreKeyWords(const set<string>& key_words, istream& input) {
  ...
  while (numOfCurString != 10'000)
  for (string line; getline(input, line); ) {
    streams[numOfCurStream] << line << '\n';
    numOfCurStream = (++numOfCurStream) % numOfStreams;
  }
