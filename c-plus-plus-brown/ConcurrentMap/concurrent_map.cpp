...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count)
    : _data(bucket_count), _mtxs(bucket_count)
  {
  }

  WriteAccess operator[](const K& key) {
    auto& bucket = _data[GetBucketIndex(key)];
    auto& mutex = _mtxs[GetBucketIndex(key)];
    return { key, mutex, bucket };
  }
  
  ReadAccess At(const K& key) const {
    if (!Has(key))
      throw std::out_of_range("");
    const auto& bucket = _data[GetBucketIndex(key)];
    auto& mutex = _mtxs[GetBucketIndex(key)];
    return { key, mutex, bucket };
  }
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (size_t idx = 0; idx < _data.size(); ++idx) {
      auto g = Lock(_mtxs[idx]);
      result.insert(begin(_data[idx]), end(_data[idx]));
    }
    return result;
  }
...
private:
  size_t GetBucketIndex(const K& key) const {
    return _hasher(key) % _data.size();
  }

  vector<MapType> _data;
  mutable vector<mutex> _mtxs;
  Hash _hasher;
};
