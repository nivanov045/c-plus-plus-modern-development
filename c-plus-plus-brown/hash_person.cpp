...
struct AddressHasher {
  size_t operator()(const Address& value) const {
    const size_t someVal = 17;
    return stringHash(value.city) * someVal * someVal + stringHash(value.street) * someVal + intHash(value.building);
  }
};
...
struct PersonHasher {
  size_t operator()(const Person& value) const {
    const size_t someVal = 37;
    return addressHash(value.address) * someVal * someVal * someVal + stringHash(value.name) * someVal * someVal +
      intHash(value.height) * someVal + doubleHash(value.weight);
  }
};
